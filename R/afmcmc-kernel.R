step_kernel <- function(X, prior, 
  ...
  if (!fix_alpha) {
    for (j in 1:alpha_repropose) {
      ...
      logr1 <- (loglik_prop - loglik_now) + 
        r1_prior_diff(a_prop, alpha, alpha_m, alpha_s)
  ...
  if (!fix_mu) {
    ...
    logr2 <- (loglik_prop - loglik_now) + 
      r2_prior_diff(mu_prop, mu, Sigma, prior)
  ...
  if (!fix_Sigma) {
    ...
    logr3 <- (loglik_prop - loglik_now) + 
      r3_prior_diff(Sigma_prop, Sigma, mu, prior)
